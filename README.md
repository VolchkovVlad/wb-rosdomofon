...
- `Статус замка`<br>
  - 0: Открыт - Если API вернул нам: `"activityResult": "opendoor_complete"` на команду открытия двери.
  - 1: Закрыт - При загрузке драйвера и через 5 секунд после открытия двери.
  - 2: Зажат  - Когда API отвечает нам что запрос не корректный.
  - 3: Не известен
- `Замок`<br>
  - 0: Открыть
  - 1: Закрыть
