...
- `Замок`<br>
  ...
  - 1: Закрыть
- `Камера HLS`
  - Ссылка на HLS поток с камеры (Для открытия потока необходим токен доступа)
- `Камера HLS Прокси`
  - Ссылка на HLS поток с камеры, проксируемый через контроллер. (Токен доступа обновляется автоматически. Ссылка не изменяется. Можно добавить в go2rtc)
...
Если у вас версия 1.1.0 и выше, а на контроллере установлен релиз wb-2606 и новее. То в вкладе `Интеграции` вы заметите `РосДомофон`
Для открытия конфигурационного файла необходимо в веб интерфейсе контроллера Wiren Board:
![Открытие конфигурационного файла](docs/img/WebUIWBIntegrations.png)
...
После внесения изменений нажмите кнопку "Записать". Драйвер перезагрузится и продолжит работу с новыми настройками.
...
7) Выбираем контроллер "Камеры", в правом верхнем углу нажимаем "+"
8) Вводим: RTSP ссылку, логин и пароль от камеры
---
